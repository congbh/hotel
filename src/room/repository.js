...
const { TABLES: { ROOM } } = require('../config')
...
module.exports = function ({ connection }) {
  ...
  const load = async (id) => {
    let result = await connection(ROOM)
      .where({ id })
      .select()
      .then(data => data[0])
    return result
  ...
  const list = async (filters = {}, limit = null, offset = null) => {
    ...
    if (!_.isNull(limit) && !_.isNull(offset)) {
      result = await connection(ROOM)
        .select()
        .where(filters)
        .limit(limit)
        .offset(offset)
    } else {
      result = await connection(ROOM).select().where(filters)
    }
    return result
  }
