...
const { TABLES: { SERVICE } } = require('../config')
...
module.exports = function ({ connection }) {
  ...
  const create = async (object) => {
    ...
    let ids = await connection(SERVICE)
      .returning('id')
      .insert(object)
  ...
  const load = async (id) => {
    let result = await connection(SERVICE)
      .where({ id })
      .select()
      .then(data => data[0])
    return result
  ...
  const list = async (filters = {}, limit = null, offset = null) => {
    ...
    if (!_.isNull(limit) && !_.isNull(offset)) {
      result = await connection(SERVICE)
        .select()
        .where(filters)
        .limit(limit)
        .offset(offset)
    } else {
      result = await connection(SERVICE).select().where(filters)
    }
    return result
  }
